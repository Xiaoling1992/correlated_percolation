...
int N=10,M=10;   //N, the number of rows, M, the number of columns. L: the width of the frame.
...
double tau=1, tau_off=0, phi, rho=0, phi_deviation=0.1;

//int height, width, heights[]={N},widths[]={M};
double phis[]={0.5, 0.43}, rhos[]={0.0};
...
for(iphi=0; iphi<Len_phis; iphi++){ 	
    ...
    strcat(filename,"THW10_map.dat");	strcat(filename2, "THW10_Clu.dat");
...
for(ip=0;ip<NumRep; ip++){         
  ...
  PriMatD(tauNoise, N, M, N, M, fptMap); //print the last map;
  fclose(fptMap);  
