...
int CluGen_trkA(double *nvth,int height,int width,double phi,double rho, double lambda, double DLow,double DHigh);
...
double tau=1, tau_off=0, phi, rho=0, lambda=10000, phi_deviation=0.0005;

//int height, width, heights[]={N},widths[]={M};
double phis[]={0.1302}, rhos[]={0, 0.3, 0.70};
...
for(iphi=0; iphi<Len_phis; iphi++){ 	
    ...
    strcat(filename,"TH35W230_L10000_map.dat");	strcat(filename2, "TH35W230_L10000_Clu.dat");
...
for(ip=0;ip<NumRep; ip++){         
  ...
  //CluGen_G5_1(double *nvth,int height,int width,double phi,double rho,double DLow,double DHigh,
              //double alpha, double beta, double a1, double a2, double a3)
  CluGen_trkA(tauNoise, N, M, phi, rho, lambda, tau_off, tau);
...
double pon, poff, ponon,ponoff, poffon, poffoff, pononon, pononoff, ponoffon, ponoffoff, possibility[width],Prenvth[HeiMat*WidMat];
 ...
 int i,j,m;
double decPar, pon, poff, ponon,ponoff, poffon, poffoff;
  decPar= exp(-1/ lambda);
  for(i=0;i<height;i++){
	if(i==0){    //row 0
		pon= phi;
		poff=1- pon;
		for(j=0;j<width;j++)
		    nvth[j]=AssignPhi_MT(pon,DLow,DHigh);
	 }
				
	else if(i>= 1){  //row 1, 2...34
		pon=phi* exp(-i/lambda);  //the fraction in the current row.
		poff=1- pon;
		ponon=decPar*pon/ (pon+ (1-rho)*(decPar- pon) );
		poffon=1- ponon;
		ponoff=(1-rho)*ponon;
		poffoff= 1-ponoff;		
		...
		for(j=0;j<width;j++){
			m=i*width+j;
			if(nvth[m-width]==DHigh)   
			 nvth[m]=AssignPhi_MT(ponon,DLow,DHigh);				
			else
			 nvth[m]=AssignPhi_MT(ponoff,DLow,DHigh);    
		}
	}
