...
template<class T>
bool printVectorScreen( vector<T>& vector_print){
...
bool Biofilm<T>::generateBiofilm(double phi,double rho,double mobility){
    ...
    //cout<<endl;
    //cout<<"Before initiating"<<endl;
    //printVectorScreen<T>(appetizer);
    //printVectorScreen<T>(tem);
    //cout<<endl;
...
int main(){
    int hei_bio= 2, wid_bio= 6, repeats=1;
    double phi=0.5, rho_dynamic=0.38, rho_spatial=0.17, mobilities[]= {0}; 
    ...
    //for( int i=0; i< sizeof(mobilities)/sizeof(mobilities[0] ); ++i){
        ...
        for(int irepeat=0; irepeat<repeats; ++irepeat){
            ...
            Biofilm<int> biofilm(hei_bio, wid_bio, 1, 0, 2, 1); //so that x_i*x_j shows they are same of different
            biofilm.generateBiofilm(phi, rho_dynamic, mobilities[0] );
            
            rho_spatial= biofilm.getSpaCorY();
            cout<< rho_spatial <<endl;
            
            
            //covariance_y_tem= biofilm.measureCorFunY( phi);
            //printVectorScreen(covariance_y_tem);
            
            covariance_x_tem= biofilm.measureCorFunX( phi);
            printVectorScreen(covariance_x_tem);
            ...
            if(irepeat==0)
                biofilm.print( "/home/xiaoling/correlated_percolation/data/Bio4_test.dat" );
        }
        //for(int i=0; i<hei_bio; ++i)
