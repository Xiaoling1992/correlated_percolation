...
int N=35,M=35;   //N, the number of rows, M, the number of columns. L: the width of the frame.
...
for (i=0; i<126; i++){ //0, 4/1000, 0.5
	phis[i]=i*4.0 /1000.0;}
	
for (i=0; i<101; i++){ //0, 1/100, 1;
	rhos[i]=1.0*i /100.0;}
	
printf("%lf %lf %lf", rhos[0], rhos[1], rhos[2]); 
...
for(ip=0;ip<NumRep; ip++){         
  ...
  printf("rho %lf  phi %lf connection %lf\n",rho, phi, connection[iphi]);  
}  //end of loop iphi
