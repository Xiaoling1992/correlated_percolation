...
double Biofilm<T>::generateBiofilm_Push_Inf(double phi,double rho, double mean, double sigma){      
    ...
    for(int i=0; i< 10; ++i){
        int mother= pq.top();
        double moth_divtime= pq.getTime(mother);
...
bool Biofilm<T>::writeNeighbor(T onoff, double div_time, int mother, int direction, vector<int>& reverse_dir, T *biofilm_tem){
    ...
    if( neighbor>=0 && neighbor< wid_bio* hei_bio && biofilm_tem[neighbor]== MAXRAND){
        ...
        if(direction != 0){ //Send daughter to the reverse didrection
            writeNeighbor(onoff, div_time, mother, reverse_dir[direction], reverse_dir, biofilm_tem);
            return true;
        }
        else{ //direction==0
           ...
           writeNeighbor(mother_onoff, mother_div, mother, reverse_dir[direction], reverse_dir,  biofilm_tem);
           return true;
            }
    }
        ...
        biofilm_tem[neighbor]= onoff;
        pq.push(  neighbor, div_time  );        
        writeNeighbor(neighbor_onoff, neighbor_div, neighbor, direction, reverse_dir,  biofilm_tem);     
        return true;  
        }       
    }
