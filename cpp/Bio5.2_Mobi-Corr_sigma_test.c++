...
public:
    ...
    bool generateBiofilm_GR(double phi,double rho, double mean, double sigma);
...
template<class T>
bool Biofilm<T>::generateBiofilm_GR(double phi,double rho, double mean, double sigma){
    double pon=phi;
    double poff=1-pon;
	double ponon=rho+ (1-rho)* pon;   //p(d:on|m:on): rho=0, no spatial correlation. rho=1, perfect lineage inheritance
	double poffon=1-ponon;	
    ...
    while(pq.empty() == false){  //There are still cells which didn't divide.
        ...
        cout<<"mother is "<< k << "div time "<< mother_div_time ;    
...
int main(){
    ...
    double phi=0.43, rho_dynamic=1, rho_spatial=0.17, sigmas[]= {  0.1  }; 
    ...
    //vector<double> covariance_x(wid_bio, 0);
     ...
     for( int i=0; i< sizeof(sigmas)/sizeof(sigmas[0] ); ++i){
        ...
        for(int irepeat=0; irepeat<repeats; ++irepeat){
            ...
            bool success= biofilm.generateBiofilm_GR(phi, rho_dynamic, 1.0, sigmas[i] );
            
            //if (num_cells < (100+ hei_bio)* wid_bio )
            //    cout<< num_cells <<" cells out of "<< (hei_bio+100)*wid_bio <<endl;
            
            biofilm.print("/home/xiaoling/correlated_percolation/test/Bio5.2_test.dat");
            rho_spatials[i]+= biofilm.getSpaCorY();
        }
        rho_spatials[i]/= repeats;
    }    
    
    //printVector<double>(rho_spatials, "/home/xiaoling/correlated_percolation/data/Bio5.2_rho-spa_sigma.dat");
