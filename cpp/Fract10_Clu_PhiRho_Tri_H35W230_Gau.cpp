...
int  Len_phi_means=sizeof(phi_means)/sizeof(phi_means[0]), Len_sigmas=sizeof(sigmas)/sizeof(sigmas[0]), Len_rhos=sizeof(rhos)/sizeof(rhos[0]), NumRep=1000;
...
for(ip=0;ip<NumRep; ip++){         
  ...
  //CluGen_G5_1(double *nvth,int height,int width,double phi,double rho,double DLow,double DHigh,
              //double alpha, double beta, double a1, double a2, double a3)
  phi= gaussrand_K()* sigma+ phi_mean; 
  double phi_actual;
  do{
	phi_actual=0;
