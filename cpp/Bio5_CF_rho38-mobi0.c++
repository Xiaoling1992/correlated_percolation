...
public:
    ...
    int generateBiofilm_GR(double phi,double rho, double mean, double sigma, double mobi_left, double mobi_right);
...
bool Biofilm<T>::generateBiofilm(double phi,double rho,double mobility){
    ...
    for(int i=1; i<  hei_more; ++i){ //Initiate appetizer for 30 times so that it reach a steady state.
...
bool operator<(const KDivision& point1, const KDivision& point2){//use const to protect the input variable.
	return point1.division_time> point2.division_time;  //We can return point1.division_time> point2.distance to reverse the sorting. (descending sorting or ascending priorityqueue)
}
...
template<class T>
int Biofilm<T>::generateBiofilm_GR(double phi,double rho, double mean, double sigma, double mobi_left, double mobi_right){
    double pon=phi;
    double poff=1-pon;
	double ponon=rho+ (1-rho)* pon;   //p(d:on|m:on): rho=0, no spatial correlation. rho=1, perfect lineage inheritance
	double poffon=1-ponon;	
    ...
    while(pq.empty() == false){  //There are still cells which didn't divide.
        ...
        if (j%2==1){ //in odd column 1, 3, 5, 7, wid_bio-1
            left= k-1;
            bottom= k+ wid_bio;
            
            if(j== wid_bio-1 ) //last column
                right= k+1- wid_bio;
            else
                right=k+1;
        }     
            ...
            if(j== 0 ) //last column
                left= k-1+ wid_bio+ wid_bio;
            else
                left=k-1+ wid_bio;            
            }
              
        free_space= (left>=0 && left< length_bio_tem && biofilm_tem[left]==MAXRAND)+ (right>=0 && right< length_bio_tem && biofilm_tem[right]==MAXRAND)+ (bottom>=0 && bottom< length_bio_tem && biofilm_tem[bottom]==MAXRAND);
        
        if (free_space==0)
            continue;
        //if the mother has free neighboring space, determine a location for its daughter.
        do{
            double rand_tem= RanGen.randExc(); //[0, 1)
            if(rand_tem< mobi_left)
                daughter= left;
            else if(rand_tem< 1-mobi_right)  //Attention: mobi_left, mobi_right can't be 0 or 0.5
                daughter= bottom;
            else
                daughter= right;
            } while (  daughter< 0 || daughter>= length_bio_tem || biofilm_tem[daughter] !=MAXRAND );    
...
int main(){
    ...
    double phi=0.43, rho_dynamic=0.38, rho_spatial=0.17, sigmas[]= { 0.0}; 
    //vector<double> rho_spatials( sizeof(sigmas)/sizeof(sigmas[0] ), 0 );        
     //for( int i=0; i< sizeof(sigmas)/sizeof(sigmas[0] ); ++i){
        ...
        //for(int irepeat=0; irepeat<repeats; ++irepeat){
            ...
            ////int num_cells= biofilm.generateBiofilm_GR(phi, rho_dynamic, 1.0, sigmas[i], 1.0/3.0, 1.0/3.0);
            //int num_cells= biofilm.generateBiofilm_GR(phi, rho_dynamic, 1.0, sigmas[i], 0, 0);
    ...
    //printVector<double>(rho_spatials, "/home/xiaoling/correlated_percolation/data/Bio5_rho-spa_mobility_sigma_2.dat");
    ...
    //for( int i=0; i< sizeof(mobilities)/sizeof(mobilities[0] ); ++i){
        ...
        for(int irepeat=0; irepeat<repeats; ++irepeat){
            ...
            Biofilm<int> biofilm(hei_bio, wid_bio, 1, 0, 2, 1);  //hei_bio, wid_bio, on, off, hei_cell, wid_cell
            //int num_cells= biofilm.generateBiofilm_GR(phi, rho_dynamic, 1.0, sigmas[0], 0, 0);
            //int num_cells= biofilm.generateBiofilm_GR(phi, rho_dynamic, 1.0, sigmas[i], 0, 0);
            biofilm.generateBiofilm(phi, rho_dynamic, 0);
            ...
            if(irepeat==0)
                biofilm.print( "/home/xiaoling/correlated_percolation/data/Bio5_biofilm_Rho38Mobi0.dat" );
        }
        for(int i=0; i<hei_bio; ++i)
    ...
    printVector<double>(covariance_y, "/home/xiaoling/correlated_percolation/data/Bio5_CF_y_r3000_Rho38Mobi0.dat");
    printVector<double>(covariance_x, "/home/xiaoling/correlated_percolation/data/Bio5_CF_x_r3000_Rho38Mobi0.dat");
