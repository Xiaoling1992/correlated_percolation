...
template<class T>
bool printVector2D(vector< T > output, int width, string output_path){
...
public:
    ...
    bool legalCell(int location, int maximum);
    ...
    while(pq.empty() == false){    //There are still cells which didn't divide.
        ...
        for(int m=0; m< sizeof(directions3)/sizeof(directions3[0]) ; ++m){   //consider only the direction of 1, 
            ...
            for(int n=0; n<=push; ++n){ //neighbor, next_neighbor, next next neighbor
                legal= legal&& legalCell( neighbors[n][direc],  length_bio_tem-1 ); //whether this cell and its front neighbors are legal.
        ...
        //cout<<"    nei"<<neighbors[0][direction]<<"  next nei "<<neighbors[1][direction]<<" dau_onoff "<<dau_onoff<<", dau div "<<dau_divtime<<endl;
...
int Biofilm<T>::getNeighbor(int mother, int direction){
    //mother is between -inf to inf
    int i= (i>=0)? (mother /wid_bio): ( (mother+1) /wid_bio- 1      );  //row i:
    int j= mother- wid_bio* i; //(i>=0)? (mother %wid_bio): (mother %wid_bio+ wid_bio);  //column j
...
template<class T>
bool Biofilm<T>::legalCell(int location, int maximum){
    return location>=0 and location<= maximum;
    }
...
int main(){
    int hei_bio= 35, wid_bio= 230, repeats=1, push=1;  //1. size;
    double phi=0.43, rho_dynamic=0.38, rho_spatial=0.17, sigmas[]= { 0.01 }; 
