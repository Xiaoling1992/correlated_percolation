...
double Biofilm<T>::generateBiofilm_Push(double phi,double rho, double mean, double sigma){
    ...
    //Pop the mother whose division time is the earliest. Decide whether it has free neighboring spaces. 
    int bottom, free_space, daughter;
    ...
    while(pq.empty() == false){  //There are still cells which didn't divide.
        ...
        next_neighbors[1]= neighbors[0]+ wid_bio; //bottom bottom
        ...
        cout<< "mother cell is "<<k<<endl;
        //1. test neighbors and next_neighbors
        cout<<"    neighbors are  ";
        for(int m=0; m<6; ++m)
            cout<< neighbors[m] <<"  ";
        cout<<endl<<"next neighbors are  ";
        for(int m=0; m<6; ++m)
        ...
        for(int i=0; i< 6; ++i){
            if ( neighbors[i]>=0 && neighbors[i]< length_bio_tem && biofilm_tem[ neighbors[i] ]==MAXRAND ) //the location is legal and free.
                avai_neighbors.push_back(  neighbors[i] );   
            else if( next_neighbors[i]>=0 && next_neighbors[i]< length_bio_tem && biofilm_tem[ next_neighbors[i] ]==MAXRAND)  //the location of next neighbor is legal and free.
                avai_neighbors.push_back(  neighbors[i] );
            }
        cout<<endl<<"    Available neighbors are  ";
        for(int m=0; m< avai_neighbors.size(); ++m)
            cout<< avai_neighbors[m] <<"  ";
        cout<<endl;
        ...
        daughter= avai_neighbors[ index_daughter ]; //pick up a daughter: [0, #daughters -1]
        ...
        if( biofilm_tem[daughter]== MAXRAND){ //  if the neighboring location is free and legal 
            ...
            biofilm_tem[ next_neighbors[index_daughter] ]= biofilm_tem[ daughter ]; 
            pq.erase(daughter); //erase ( daughter, nei_div_time);
            pq.push( next_neighbors[index_daughter], nei_div_time ); //push the transfered cell into pq
...
int main(){
    ...
    double phi=0.43, rho_dynamic=1, rho_spatial=0.17, sigmas[]= { 1 }; 
    ...
    //vector<double> covariance_x(wid_bio, 0);
     ...
     for( int i=0; i< sizeof(sigmas)/sizeof(sigmas[0] ); ++i){
        ...
        for(int irepeat=0; irepeat<repeats; ++irepeat){
            ...
            perc_daugh_bottom[i]+= biofilm.generateBiofilm_Push(phi, rho_dynamic, 1.0, sigmas[i] );
            
            //if (num_cells < (100+ hei_bio)* wid_bio )
            //    cout<< num_cells <<" cells out of "<< (hei_bio+100)*wid_bio <<endl;
            
            biofilm.print("/home/xiaoling/correlated_percolation/test/Bio6_test-neighbors.dat");
    ...
    //printVector<double>(perc_daugh_bottom, "/home/xiaoling/correlated_percolation/data/Bio6_perdaubot_sigma.dat");
