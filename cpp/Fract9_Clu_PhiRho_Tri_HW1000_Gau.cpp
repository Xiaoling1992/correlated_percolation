...
int N=1000,M=1000;   //N, the number of rows, M, the number of columns. L: the width of the frame.
...
double tau=1, tau_off=0, phi, phi_mean, sigma=0.023, rho, phi_deviation=0.0005;

//int height, width, heights[]={N},widths[]={M};
double phi_means[]={0.441}, rhos[]={0.0, 0.17, 0.38};


int  Len_phi_means=sizeof(phi_means)/sizeof(phi_means[0]), Len_rhos=sizeof(rhos)/sizeof(rhos[0]), NumRep=1000;
int i,j, iphi_mean, irho, ip, iheight; //i, j is only used for any short, self-closed loop. 
...
for(iphi_mean=0; iphi_mean<Len_phi_means; iphi_mean++){ 	
    ...
    strcpy(filename, "Fra9");     
    strcat(filename,"Phi");   sprintf(num, "%d",(int)(phi_mean*10000+EPSILON) );strcat(filename,num);
    strcat(filename,"Rho");   sprintf(num, "%d",(int)(rho*100+EPSILON));strcat(filename,num); 
    strcpy(filename2, filename);
    strcat(filename,"THW1000_map_Gau.dat");	strcat(filename2, "THW1000_Clu_Gau.dat");
